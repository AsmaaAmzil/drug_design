...
#         except Exception as e:
#             print(f"Error embedding query: {e}")
#             embedding = np.zeros(4096, dtype=np.float64)  # ← fix: not None
#         return embedding
...
#             if not all(len(e) == len(embedding_list[0]) for e in embedding_list):
#                 raise ValueError("Inhomogeneous embeddings")
#         except Exception as e:
#             print(f"Error embedding documents: {e}")
#             embedding_list = [np.zeros(4096, dtype=np.float64).tolist() for _ in query_list]
#         return embedding_list
class BGEM3Embedding:
    ...
    def request_embedding(self, input_):
        payload = {
            "model": "jina-embeddings-v3",
            "input": input_ if isinstance(input_, list) else [input_],
            "dimensions": 1024,
            "encoding_format": "float"
        }
        headers = {
    ...
    def embed_query(self, query: str):
        ...
        try:
            ...
            print(f"Error embedding query: {e}")
            embedding = np.zeros(1024, dtype=np.float64)
        return embedding
    ...
    def embed_documents(self, query_list: List[str]):
        ...
        try:
            ...
            print(f"Error embedding documents: {e}")
            embedding_list = [np.zeros(1024, dtype=np.float64).tolist() for _ in query_list]
        return embedding_list
